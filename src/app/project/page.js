        ...
        <div className="flex items-center">
          ...
          <div className="ml-auto flex items-center space-x-0.5 pr-1"> 
            <button
              ...
              onClick={(e) => { e.stopPropagation(); setParentFolderForModal(item.id); setIsCreateFolderModalOpen(true); }}
    ...
    <div className="p-4 bg-white border border-gray-200 rounded-lg shadow-sm hover:shadow-md transition-shadow">
      <div className="flex items-center space-x-3">
        ...
        <div>
          ...
          <p className="text-sm text-gray-600 font-medium">Rol: {projectUser.roleCode}</p>
        </div>
      </div>
    </div>
  );
...
function MembersView({ projectId }) {
  ...
  if (loadingMembers) return <Loader text="Cargando integrantes..." />;
  if (membersError) return <p className="text-center p-4 text-red-600 bg-red-100 rounded-md">{membersError}</p>;
        ...
        {members.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {members.map((projectUser) => ( 
              <UserCard key={projectUser.id} projectUser={projectUser} /> 
            ))}
          </div>
        ) : (
